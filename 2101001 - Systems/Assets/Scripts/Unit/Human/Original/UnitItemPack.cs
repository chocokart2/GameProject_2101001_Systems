...
public class UnitItemPack : MonoBehaviour
{
    ...
    public Item[] inventory; // 아이템을 저장하는 어레이입니다. 유닛 타입이 human이면 요소가 3개이고, 머신이면 요소가 1개입니다
    ...
    void Update()
    {
        ...
        if(inventory != null)
        {
            ...
            {
                inventory[inventoryIndex].Update(Time.deltaTime);
            }
        }
    ...
    public class ItemData
    {
        ...
        public GameManager.AttackClassData[] attackClassDatas; // 이 아이템의 공격 클래스입니다.
    }
    ...
    public static ItemData NewItemData()
    {
        ...
        returnValue.attackClassDatas = new GameManager.AttackClassData[] { GameManager.NewAttackClassData() };
        return returnValue;
    }
    ...
    public abstract class Item
    {
        ...
        public List<GameManager.AttackClass> attackClasses; // 이 아이템을 이용해 공격을 할 시, 사용되는 공격 클래스입니다.
        ...
        public Item(GameObjectList gameObjectList)
        {
            ...
            this.attackClasses = new List<GameManager.AttackClass>();
        }
        public Item(GameObjectList gameObjectList, ItemData data) : this(gameObjectList)
        {
            ...
            if (isRealItem == true) // 데이터 불러오기 작업을 실시합니다.
            {
                ...
                for (int atkClassIndex = 0; atkClassIndex < data.attackClassDatas.Length; atkClassIndex++)
                {
                    this.attackClasses.Add(new GameManager.AttackClass(data.attackClassDatas[atkClassIndex]));
                }
            }
            else
            {
                // 기본 아이템에서 subItem이 필요하지 않습니다.
                this.attackClasses.Add(new GameManager.AttackClass(1.0f, 1.0f, 1.0f, new GameManager.chemical("unknown", 1.0f)));
            }
        }
        public virtual void GetData(ref ItemData itemData) // 이 함수는 반드시 상속해서 정의해주세요.
        {
            ...
            returnValue.attackClassDatas = new GameManager.AttackClassData[attackClasses.Count];
            returnValue.attackClassDatas = GameManager.AttackClass.toAttackClassDataArray(attackClasses.ToArray());
            itemData = returnValue;
        }
    }
    #region Blank
    ...
    public class Pistol : Item
    {
        ...
        public Pistol(GameObjectList gameObjectList) : base(gameObjectList)
        {
            ...
            this.AmmoLoaded = 4;
            this.attackClasses = new List<GameManager.AttackClass>();
            this.attackClasses.Add(new GameManager.AttackClass(10.0f, 30.0f, 10.0f, new GameManager.chemical("iron1", 1.0f), new GameManager.chemical("iron2", 1.0f)));
        }
        public Pistol(GameObjectList gameObjectList, ItemData data) : base(gameObjectList, data)
        {
            if(data.isRealItem == false)
            {
                ...
                this.AmmoLoaded = 4;
                this.attackClasses[0] = new GameManager.AttackClass(10.0f, 30.0f, 10.0f, new GameManager.chemical("iron1", 1.0f), new GameManager.chemical("iron2", 1.0f));
            }
        ...
        public override float Use(Vector3 vector3)
        {
            if(AmmoLoaded > 0)
            {
                gameObjectList.PistolBulletShot(attackClasses[0], vector3);
                AmmoLoaded--;
                return 0.3f;
            }
            else
    ...
    public class Knife : Item
    {
        ...
        {
            this.attackClasses = new List<GameManager.AttackClass>();
            this.attackClasses.Add(new GameManager.AttackClass(10.0f, 30.0f, 3000.0f, new GameManager.chemical("iron1", 1.0f)));
        }
        ...
        public override float Use(Vector3 vector3)
        {
            gameObjectList.KnifeBladeStab(this.attackClasses[0], vector3);
    ...
    public void ItemUse(Vector3 vector3)
    {
        ...
        Cooldown = inventory[inventoryIndex].Use(vector3);

    }
    public void ItemSkill(Vector3 vector3)
    {
        if (Cooldown > 0.0f) return;
        Cooldown = inventory[inventoryIndex].Skill(vector3);
    }
    public void ItemSupply(Vector3 vector3)
    {
        if (Cooldown > 0.0f) return;
        Cooldown = inventory[inventoryIndex].Supply(vector3);
    }
    ...
    public void InventorySet()
    {
        ...
        inventory[0] = new Pistol(gameObjectList);
        inventory[1] = new Knife(gameObjectList);
        inventory[2] = new BuildTool(gameObjectList);
    }
    public void InventorySet(Item item1, Item item2, Item item3)
    {
        inventory[0] = item1;
        inventory[1] = item2;
        inventory[2] = item3;
    ...
    public void InventorySetForMachine(Item item)
    {
        GameObjectList gameObjectList = GetComponent<GameObjectList>();
    ...
    public void InventorySet(UnitItemPackData data)
    {
        ...
        //InventorySet(item1, item2, item3);
        //inventory[0].subItem = subItem1;
        //inventory[1].subItem = subItem2;
        //inventory[2].subItem = subItem3;
    }
    public void InventorySet(int index, Item item)
    {
