            ...
            if (Physics.Raycast(ray, out hit, 50.0f, unitLayerMask))
            {
                if (hit.transform.CompareTag("Unit"))
                {
                    ...
                    if (hit.collider.gameObject.GetComponent<UnitBase>().unitBaseData.unitType == "human")
                    {
                        ...
                        if (GameObject.Find("GameManager").GetComponent<GameManager>().currentFieldData.playerTeamName.Equals(hit.collider.gameObject.GetComponent<UnitRole>().roleForEditMode.teamName)) // 자신의 팀과 게임오브젝트의 팀이 똑같은지 체크
                        {
